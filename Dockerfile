...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl make libffi-dev openssl-dev libjpeg-turbo-dev zlib-dev \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant==0.93.1 \
                                          PyMySQL \
                                          ...
                                          zeroconf==0.22.0 colorlog==4.0.2 pysonos==0.0.12 plexapi==3.0.6 PyXiaomiGateway==0.12.3 \
                                          ...
                                          PyQRCode==1.2.1 pyotp==2.2.7 aioesphomeapi==2.0.1 python-forecastio==1.4.0 \
