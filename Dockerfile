...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl make libffi-dev openssl-dev libjpeg-turbo-dev zlib-dev \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant==0.103.5 \
                                          PyMySQL \
                                          aiohttp_cors==0.7.0 distro==1.4.0 \ 
                                          home-assistant-frontend==20191204.1 \
                                          mutagen==1.43.0 netdisco==2.6.0 slixmpp==1.4.2 sqlalchemy==1.3.11 xmltodict==0.12.0 \
                                          zeroconf==0.24.0 colorlog==4.0.2 pysonos==0.0.24 plexapi==3.3.0 PyXiaomiGateway==0.12.4 \
                                          python-miio==0.4.7 yeelight==0.5.0 broadlink==0.12.0 coinmarketcap==5.0.3 aiohue==1.9.2 \
                                          ...
                                          wakeonlan==1.1.6 aioftp==0.12.0 PyNaCl==1.3.0 influxdb==5.2.3 hass-nabucasa==0.30 \
                                          workalendar && \
