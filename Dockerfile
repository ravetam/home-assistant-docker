...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl make libffi-dev openssl-dev libjpeg-turbo-dev zlib-dev \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant==0.94.4 \
                                          PyMySQL \
                                          aiohttp_cors==0.7.0 distro==1.4.0 \ 
                                          home-assistant-frontend==20190604.0\
                                          mutagen==1.42.0 netdisco==2.6.0 slixmpp==1.4.2 sqlalchemy==1.3.3 xmltodict==0.12.0 \
                                          zeroconf==0.23.0 colorlog==4.0.2 pysonos==0.0.12 plexapi==3.0.6 PyXiaomiGateway==0.12.3 \
                                          python-miio==0.4.5 yeelight==0.5.0 broadlink==0.10.0 coinmarketcap==5.0.3 aiohue==1.9.1 \
                                          pyowm==2.10.0 slacker==0.13.0 feedparser-homeassistant==5.2.2.dev1 \
                                          construct==2.9.45 paho-mqtt==1.4.0 alpha_vantage==2.1.0 \
                                          PyQRCode==1.2.1 pyotp==2.2.7 aioesphomeapi==2.1.0 python-forecastio==1.4.0 \
