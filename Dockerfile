...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl make libffi-dev openssl-dev libjpeg-turbo-dev zlib-dev \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant==0.104.1 \
                                          PyMySQL \
                                          aiohttp_cors==0.7.0 distro==1.4.0 \ 
                                          home-assistant-frontend==20200108.0 \
                                          mutagen==1.43.0 netdisco==2.6.0 slixmpp==1.4.2 sqlalchemy==1.3.12 xmltodict==0.12.0 \
                                          zeroconf==0.24.4 colorlog==4.0.2 pysonos==0.0.24 plexapi==3.3.0 PyXiaomiGateway==0.12.4 \
