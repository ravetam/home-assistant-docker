...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl make libffi-dev openssl-dev libjpeg-turbo-dev zlib-dev \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant==0.112.2 \
                                          PyMySQL \
                                          aiohttp_cors==0.7.0 distro==1.5.0 \ 
                                          home-assistant-frontend==20200702.0 \
                                          mutagen==1.44.0 netdisco==2.7.1 slixmpp==1.5.1 sqlalchemy==1.3.17 xmltodict==0.12.0 \
                                          zeroconf==0.27.1 colorlog==4.1.0 plexapi==4.0.0 plexauth==0.0.5 plexwebsocket==0.0.11 \
                                          PyXiaomiGateway==0.12.4 pysonos==0.0.31 \
                                          python-miio==0.5.2.1 yeelight==0.5.2 broadlink==0.14.0 coinmarketcap==5.0.3 aiohue==2.1.0 \
                                          ...
                                          wakeonlan==1.1.6 aioftp==0.12.0 PyNaCl==1.3.0 influxdb-client==1.6.0 influxdb==5.2.3 hass-nabucasa==0.34.7 \
                                          HAP-python==2.9.1 beautifulsoup4==4.9.0 blockchain==1.4.4 jsonpath==0.82 \
