...
RUN apk add --no-cache --update bash gcc musl-dev linux-headers curl libssl1.0 mariadb-client \
    ...
    pip3 install --no-cache-dir --upgrade homeassistant \
                                          ...
                                          python-miio==0.4.5 yeelight==0.4.3 broadlink==0.9.0 coinmarketcap==5.0.3 aiohue==1.9.1 \
